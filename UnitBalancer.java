...
class UnitBalancer {
    public static void main(String args[] ) throws Exception {
        ...
        String lowestUnit = Stream.of(arrUntits).parallel().filter(x->!relationsKeys.contains(x)).collect(Collectors.joining());
        List<String> sortedUnitsLowToHigh = new LinkedList();
        sortedUnitsLowToHigh.add(lowestUnit);
        
        for(int index = 0;index<M-1;index++){
        	String low = lowestUnit;
        	lowestUnit = mapRelations.entrySet().stream().parallel().filter(x->x.getValue().contains(low)).map(x->x.getKey()).collect(Collectors.joining());
        	sortedUnitsLowToHigh.add(lowestUnit);
        }
        
        if(sortedUnitsLowToHigh!=null && !sortedUnitsLowToHigh.isEmpty() && mapRelations!=null && !mapRelations.isEmpty()){
